...
app.get('/fotos/:sort?', function(req, res, next) {
  ...
  return invoke(function(data, callback) {
    ...
    if (sort === 'ultimas') {
      photos.desc('created_at');
    }
    if (sort === 'top') {
      photos.desc('views', 'created_at');
    }
...
app.post('/login', passport.authenticate('local', {
  ...
  url = _.size(flash) ? _.first(flash) : '/profile';
...
app.get('/:user/:slug/editar', middleware.auth, function(req, res) {
  ...
  return invoke(function(data, callback) {
    ...
    if (!data) {
      photo.resize_photos(404)(req, res);
    }
    if (!_.isEqual(user._id, req.user._id)) {
