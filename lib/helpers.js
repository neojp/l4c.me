...
module.exports = {
  ...
  logger_format: function(tokens, req, res) {
    var color, d, date, status;
    status = res.statusCode;
    color = status >= 500 ? 31 : status >= 400 ? 33 : status >= 300 ? 36 : 32;
    d = moment();
    date = d.format('YYYY/MM/DD HH:mm:ss ZZ');
    return '' + '\033[' + color + 'm' + res.statusCode + ' \033[90m' + date + '  ' + ' \033[90m' + req.method + ' \033[0m' + req.originalUrl + ' \033[90m' + (new Date - req._startTime) + 'ms' + '          \033[90m' + tokens['remote-addr'](req) + '\033[0m';
  },
